...
namespace LotteryDraw.BusinessLogic.UnitTests.WinningNumberRules.Primary
{
    ...
    public class IsPrimaryWithinBoundsTests
    {
        ...
        public void Setup()
        {
            _rule = new IsPrimaryWithinBounds();
        }

        [Test]
        public void CheckDefaultState()
        {
            Assert.That(_rule.Sequence, Is.EqualTo(3));
            Assert.That(_rule.HasError, Is.False);
            Assert.That(_rule.ErrorMessage, Is.Null);
        }

        [Test]
        public void Check_Execute_WorksAsExpected_WithAllNullData()
        {
        ...
        public bool Check_Execute_WorksAsExpected_WithData(int winningNumber, int min, int max)
        {
            ...
            data.RangePrimary.Returns(range);

            var winningNumbers = Substitute.For<IWinningNumbers>();
            winningNumbers.WinningPrimaryNumbers.Returns(new List<int> { winningNumber });
