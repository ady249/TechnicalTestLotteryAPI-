...
namespace LotteryDraw.BusinessLogic.UnitTests.WinningNumberRules.Primary
{
    ...
    public class PrimaryHasDataTests
    {
        ...
        public void Setup()
        {
            _rule = new PrimaryHasData();
        }

        [Test]
        public void CheckDefaultState()
        {
            Assert.That(_rule.Sequence, Is.EqualTo(1));
            Assert.That(_rule.HasError, Is.False);
            Assert.That(_rule.ErrorMessage, Does.StartWith("Primary winning data has not been provided."));
        }
        ...
        public void Check_Execute_WorksAsExpected_WithData(string value)
        {
            ...
            winningNumbers.WinningPrimaryNumbers.Returns(value.Split(',').Select(int.Parse).ToList());
